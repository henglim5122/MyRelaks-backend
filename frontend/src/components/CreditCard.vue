                  ...
                  <span v-for="(n, $index) in otherCardMask" :key="$index">
                    <transition name="slide-fade-up">
                      <div
                        class="card-item__numberItem"
                        v-if="
                          $index > 3 && $index < 14 && cardNumber.length > $index && n.trim() !== ''
                        "
                      >
                        *
                      </div>
...
export default {
  ...
  methods: {
    formatCardNumber() {
      ...
      let cardNumberDigits = this.cardNumber.replace(/\D/g, '');

      // Limit the number of digits to 16
      cardNumberDigits = cardNumberDigits.slice(0, 16);

      // Add a space after every 4 digits
      this.cardNumber = cardNumberDigits.replace(/(.{4})/g, '$1 ').trim();
    },
